...
player = space.Joueur()
# creation du donnut
tir = space.Donnut(player)
tir.etat = "chargé"
# creation de la youtube money
listeEnnemis = []
...
while running : # boucle infinie pour laisser la fenêtre ouverte
    ...
    # le joueur
    player.deplacer()
    screen.blit(tir.image,[tir.depart,tir.hauteur]) # appel de la fonction qui dessine le policier       
    # la balle
    tir.bouger()
    screen.blit(player.image,[player.position,500]) # appel de la fonction qui dessine le policier
    # les ennemis
    for ennemi in listeEnnemis:
        ennemi.avancer()
        screen.blit(ennemi.image,[ennemi.depart, ennemi.hauteur]) # appel de la fonction qui dessine le policier
    
    player.dessiner_score(screen)
